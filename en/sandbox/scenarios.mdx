...
Add the header `X-Sandbox-Scenario: <scenario>` to any cash-in, cash-out, or OXXO call. Without the header, the sandbox uses the `success` scenario by default.
...
| `error:oxxo-expired` | `201 PENDING` | `EXPIRED` (OXXO only) |
