...
| `pending_long` | `CONFIRMED` tras ~30s |
| `rejected` | `FAILED` |
| `returned` | `RETURNED` |

`timeout` y `provider_5xx` fallan en la respuesta HTTP síncrona. `insufficient_funds` y `bad_clabe` **no aplican** a cash-in (devuelven `400 SCENARIO_NOT_APPLICABLE`).
