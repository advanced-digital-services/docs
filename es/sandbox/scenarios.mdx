...
Agrega el header `X-Sandbox-Scenario: <escenario>` a cualquier llamada de cash-in, cash-out u OXXO. Sin el header, el sandbox usa el escenario `success` por defecto.
...
| `error:oxxo-expired` | `201 PENDING` | `EXPIRED` (solo OXXO) |
